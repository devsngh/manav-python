from http import HTTPStatus
from typing import Any
from urllib.parse import quote

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.component_definition_response import ComponentDefinitionResponse
from ...models.component_definition_update import ComponentDefinitionUpdate
from ...models.http_validation_error import HTTPValidationError
from ...types import UNSET, Response, Unset


def _get_kwargs(
    component_id: str,
    *,
    body: ComponentDefinitionUpdate,
    authorization: None | str | Unset = UNSET,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}
    if not isinstance(authorization, Unset):
        headers["authorization"] = authorization

    _kwargs: dict[str, Any] = {
        "method": "put",
        "url": "/api/prompts/components/definitions/{component_id}".format(
            component_id=quote(str(component_id), safe=""),
        ),
    }

    _kwargs["json"] = body.to_dict()

    headers["Content-Type"] = "application/json"

    _kwargs["headers"] = headers
    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> ComponentDefinitionResponse | HTTPValidationError | None:
    if response.status_code == 200:
        response_200 = ComponentDefinitionResponse.from_dict(response.json())

        return response_200

    if response.status_code == 422:
        response_422 = HTTPValidationError.from_dict(response.json())

        return response_422

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ComponentDefinitionResponse | HTTPValidationError]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    component_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: ComponentDefinitionUpdate,
    authorization: None | str | Unset = UNSET,
) -> Response[ComponentDefinitionResponse | HTTPValidationError]:
    """Update Component Definition

     Update a component definition

    Args:
        component_id (str):
        authorization (None | str | Unset): Bearer token
        body (ComponentDefinitionUpdate):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[ComponentDefinitionResponse | HTTPValidationError]
    """

    kwargs = _get_kwargs(
        component_id=component_id,
        body=body,
        authorization=authorization,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    component_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: ComponentDefinitionUpdate,
    authorization: None | str | Unset = UNSET,
) -> ComponentDefinitionResponse | HTTPValidationError | None:
    """Update Component Definition

     Update a component definition

    Args:
        component_id (str):
        authorization (None | str | Unset): Bearer token
        body (ComponentDefinitionUpdate):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        ComponentDefinitionResponse | HTTPValidationError
    """

    return sync_detailed(
        component_id=component_id,
        client=client,
        body=body,
        authorization=authorization,
    ).parsed


async def asyncio_detailed(
    component_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: ComponentDefinitionUpdate,
    authorization: None | str | Unset = UNSET,
) -> Response[ComponentDefinitionResponse | HTTPValidationError]:
    """Update Component Definition

     Update a component definition

    Args:
        component_id (str):
        authorization (None | str | Unset): Bearer token
        body (ComponentDefinitionUpdate):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[ComponentDefinitionResponse | HTTPValidationError]
    """

    kwargs = _get_kwargs(
        component_id=component_id,
        body=body,
        authorization=authorization,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    component_id: str,
    *,
    client: AuthenticatedClient | Client,
    body: ComponentDefinitionUpdate,
    authorization: None | str | Unset = UNSET,
) -> ComponentDefinitionResponse | HTTPValidationError | None:
    """Update Component Definition

     Update a component definition

    Args:
        component_id (str):
        authorization (None | str | Unset): Bearer token
        body (ComponentDefinitionUpdate):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        ComponentDefinitionResponse | HTTPValidationError
    """

    return (
        await asyncio_detailed(
            component_id=component_id,
            client=client,
            body=body,
            authorization=authorization,
        )
    ).parsed
